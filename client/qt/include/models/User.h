...
struct User{
    ...
    User() : id(0), username(QString()), role(chat::UserRights::REGULAR) {}

    // Parameterized constructor
    User(int32_t userId, const QString& name, chat::UserRights userRole)
        : id(userId), username(name), role(userRole) {}
    ...
    bool operator<(const User& other) const {
        ...
        int usernameCmp = QString::compare(username, other.username, Qt::CaseInsensitive);
        if (usernameCmp != 0) {
